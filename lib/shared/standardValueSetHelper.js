...
class StandardValueSetHelper {
    constructor(connection, standardValueSetName) {
    ...
    async init() {
        ...
        await this.page.goto(`${this._connection.instanceUrl}/secur/frontdoor.jsp?sid=${this._connection.accessToken}`, { waitUntil: ["domcontentloaded", "networkidle0"] });
        ...
        const existingValues = (await this._connection.tooling.query(existingValuesQuery));
        for (const sv of existingValues.records[0].Metadata.standardValue) {
            this._existingValues.push(`${sv.label}/${sv.valueName}`);
        }
    }
    async addValue(label, apiName, statusCategory) {
        if (!this.browser) {
            await this.init();
        }
        let result = '';
...
exports.StandardValueSetHelper = StandardValueSetHelper;
//# sourceMappingURL=standardValueSetHelper.js.map